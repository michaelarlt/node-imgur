...
// module only.
imgur.IMGUR_CLIENT_ID    = process.env.IMGUR_CLIENT_ID || 'f0ea04148a54268';
imgur.IMGUR_API_URL      = process.env.IMGUR_API_URL || 'https://api.imgur.com/3/';
imgur.IMGUR_USERNAME     = null;
imgur.IMGUR_PASSWORD     = null;
imgur.IMGUR_ACCESS_TOKEN = null;
imgur.IMGUR_MASHAPE_KEY  = process.env.IMGUR_MASHAPE_KEY;
...
imgur._imgurRequest = function (operation, payload, extraFormParams) {
    ...
    imgur._getAuthorizationHeader()
        .then(function (authorizationHeader) {
            if(imgur.IMGUR_MASHAPE_KEY) {
                options.headers = {
                    Authorization: authorizationHeader,
                    'X-Mashape-Key': imgur.IMGUR_MASHAPE_KEY
                };
            } else {
                options.headers = {
                    Authorization: authorizationHeader
...
imgur._getAuthorizationHeader = function () {
    ...
    if (imgur.IMGUR_ACCESS_TOKEN) {
        deferred.resolve('Bearer ' + imgur.IMGUR_ACCESS_TOKEN);
    } else if (imgur.IMGUR_USERNAME && imgur.IMGUR_PASSWORD) {
        var options = {
            uri:      'https://api.imgur.com/oauth2/authorize',
            method:   'GET',
            encoding: 'utf8',
            qs: {
                client_id: imgur.IMGUR_CLIENT_ID,
                response_type: 'token'
            }
        };
        imgur._request(options).then(function (res) {
            ...
            options.json = true;
            options.form = {
                username: imgur.IMGUR_USERNAME,
                password: imgur.IMGUR_PASSWORD,
            ...
            imgur._request(options).then(function (res) {
                ...
                imgur.IMGUR_ACCESS_TOKEN = token.access_token;
                deferred.resolve('Bearer ' + imgur.IMGUR_ACCESS_TOKEN);
            }).catch(function (err) {
                deferred.reject(err);
            });
        ...
        deferred.resolve('Client-ID ' + imgur.IMGUR_CLIENT_ID);
    }
...
imgur.setCredentials = function (username, password, clientId) {
    if (clientId && typeof clientId === 'string') {
        imgur.IMGUR_CLIENT_ID = clientId;
    }
    if (username && typeof username === 'string') {
        imgur.IMGUR_USERNAME = username;
    }
    if (password && typeof password === 'string') {
        imgur.IMGUR_PASSWORD = password;
    }
}
...
imgur.setMashapeKey = function(mashapeKey) {
    if(mashapeKey && typeof mashapeKey === 'string') {
        imgur.IMGUR_MASHAPE_KEY = mashapeKey;
    }
}
/**
 * Get Mashape Key
...
imgur.getMashapeKey = function() {
    return imgur.IMGUR_MASHAPE_KEY;
}
